...
class VideoSlicer:
    ...
    async def slice(
        self, url: str, filename: str, provider_id: str = "", segment_time: int = 5
    ) -> str | None:
        ...
        try:
            ...
            command = [
                ...
                "0:a:0",
                "-map",
                "0:s:0?",  # Ambil Video, Audio, dan Teks pertama
                "-c:v",
                "copy",
                "-c:a",
                "copy",
                "-c:s",
                "webvtt",  # WebVTT adalah standar HLS
                "-f",
                "hls",
                "-hls_time",
                str(segment_time),
                ...
                os.path.join(hls_dir, "segment_%04d.ts"),
                master_playlist,
            ]
            ...
            if os.path.exists(master_playlist) and os.path.getsize(master_playlist) > 0:
                logger.info("✅ Berhasil memotong video dengan Subtitle!")
