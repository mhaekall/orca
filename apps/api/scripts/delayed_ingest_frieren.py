    ...
    for row in rows:
        ...
        async with httpx.AsyncClient(verify=False) as client:
            try:
                ...
                print(
                    f"[{ep_num}] Trigger API Timeout (Normal karena background processing jalan): {e}"
                )

        print(
            f"[{ep_num}] Menunggu 15 menit (900 detik) untuk memberi waktu Server HF memotong video..."
        )
