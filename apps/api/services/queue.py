...
class QStashPublisher:
    ...
    async def publish_ingest_task(
        ...
        is_locked = await upstash_set(
            lock_key,
            {"status": "processing", "started_at": int(time.time()), "provider": provider_id},
            ex=1800,
            nx=True,
        )
        ...
        if not is_locked:
            print(
                f"[QStash] Ingest already queued/in-progress for {anilist_id} Ep {episode_number}. Skipping deduplicate."
            )
            return
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                if res.status_code >= 400:
                    ...
                    print(
                        f"[QStash] Queued Ingestion for Ep {episode_number} successfully{delay_msg}."
                    )
            except Exception as e:
    ...
    def spawn_batch_worker(shard_id: int = 0, total_shards: int = 1):
        ...
        print(
            f"[Queue] Spawning batch ingestion (Shard {shard_id}/{total_shards}) as native asyncio task..."
        )
    ...
    async def publish_ingest_batch_task(force: bool = False):
        ...
        if not force:
            ...
            if not is_locked:
                print("[Queue] Batch ingest trigger already running. Skipping deduplicate.")
                return
        else:
            print("[Queue] Force triggering batch ingestion, bypassing lock.")
