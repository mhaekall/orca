...
export function MissionControlTab({
  api, authHeaders, logs, ingestTasks, 
  ...
  const handleAction = async (endpoint: string, label: string) => {
    setLoadingActions(prev => ({ ...prev, [endpoint]: true }));
    ...
    setLoadingActions(prev => ({ ...prev, [endpoint]: false }));
    fetchData();
  };
    ...
    <div className="space-y-6">
      <section className="bg-[#1C1C1E] rounded-[2rem] p-6 border border-white/5 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <ListButton onClick={() => handleAction('/api/v2/admin/trigger-prefetch', 'Smart Pre-fetch')} title="Execute Smart Pre-fetch" subtitle="Ongoing anime -> Telegram Swarm" icon="M13 10V3L4 14h7v7l9-11h-7z" color="bg-indigo-500" loading={!!loadingActions['/api/v2/admin/trigger-prefetch']} />
          <ListButton onClick={() => handleAction('/api/v2/admin/mass-sync', 'Mass Sync')} title="Force Mass Synchronization" subtitle="Deep sync for top 100 recent updates" icon="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" color="bg-purple-500" loading={!!loadingActions['/api/v2/admin/mass-sync']} />
          <ListButton onClick={() => handleAction('/api/v2/admin/sync-missing', 'Retry 0 Eps')} title="Retry Missing Episodes" subtitle="Re-evaluate anime with 0 episodes" icon="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" color="bg-orange-500" loading={!!loadingActions['/api/v2/admin/sync-missing']} />
          <ListButton onClick={() => {
            const id = prompt("Target AniList ID:");
            if(id) handleAction(`/api/v2/anime/${id}/debug-sync`, `Manual Scrape ID ${id}`);
          }} title="Manual ID Scrape" subtitle="Force sync a specific AniList ID" icon="M15 15l-2 5L9 9l11 4-5 2zm0 0l5 5M7.188 2.239l.777 2.897M5.136 7.965l-2.898-.777M13.95 4.05l-2.122 2.122m-5.657 5.656l-2.12 2.122" color="bg-gray-500" loading={false} />
        </div>
      </section>
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-[#1C1C1E] rounded-[2rem] border border-white/5 p-6 h-80 flex flex-col">
          <h3 className="text-sm font-semibold text-gray-400 uppercase tracking-widest mb-4 flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-purple-500" /> Active Ingestions
          </h3>
