...
async def sync_episode_db(payload: SyncEpisodePayload):
    ...
    try:
        ...
        if not anilist_id:
            raise HTTPException(
                status_code=404,
                detail=f"Anime dengan slug {payload.slug} tidak ditemukan di mapping.",
            )
        ...
        final_url = (
            payload.tg_urls[0] if isinstance(payload.tg_urls, list) else str(payload.tg_urls)
        )
        ...
        result = await database.execute(
            update_query, {"url": final_url, "anilist_id": anilist_id, "episode": payload.episode}
        )

        if result:
            print(
                f"✅ [DB Sync] Sukses update Neon DB: {payload.slug} Ep {payload.episode} -> {final_url[:30]}..."
            )
            return {"status": "success", "url": final_url}
        else:
            print("⚠️ [DB Sync] Episode tidak ditemukan di DB. Gagal update.")
