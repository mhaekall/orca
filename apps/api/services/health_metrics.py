...
async def record_ingestion_metric(
    provider_id: str, success: bool, duration_sec: float, error_type: str = None
):
    """
    ...
    try:
        ...
        new_avg = (
            ((current_avg * (attempted - 1)) + duration_sec) / attempted
            if attempted > 0
            else duration_sec
        )
...
async def record_provider_health(provider_id: str, is_reachable: bool, response_ms: float):
    """
