...
async def clear_all():
    ...
    if not CLOUDFLARE_ZONE_ID or not CLOUDFLARE_API_TOKEN:
        ...
        print(
            "💡 Silakan tambahkan kredensial tersebut di root .env agar cache CDN/Edge bisa di-purge secara otomatis."
        )
    else:
        try:
            ...
            async with httpx.AsyncClient() as http_client:
                ...
                if response.status_code == 200:
                    ...
                    print(
                        f"❌ Gagal mem-purge Cloudflare. Status: {response.status_code}, Res: {response.text}"
                    )
        except Exception as e:
