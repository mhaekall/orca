            ...
            for row in rows:
                ...
                try:
                    ...
                    res = await client.get(
                        f"https://api.jikan.moe/v4/anime/{mal_id}/statistics", timeout=10.0
                    )
                    if res.status_code == 200:
                        ...
                        logger.info(
                            f"Synced Jikan stats for mal_id={mal_id} (watching: {watching}, completed: {completed})"
                        )
                        ...
                        logger.error(
                            f"Failed to fetch Jikan stats for mal_id={mal_id}: HTTP {res.status_code}"
                        )
