...
from services.queue import enqueue_ingest_batch, enqueue_sync
...
async def smart_prefetch_episodes(force: bool = False):
    ...
    try:
        ...
        un_ingested_query = """
            ...
            LIMIT 20
        """
        un_ingested_eps = await database.fetch_all(
            un_ingested_query, values={"anime_ids": anime_ids}
        )
        ...
        logger.info(
            f"[Prefetch] Successfully queued batch ingest trigger for {queued_count} episodes."
        )
        return {
            "status": "success",
            "queued_ingestions": queued_count,
            "synced_anime": len(anime_ids),
        }
