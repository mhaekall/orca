...
async def delete_telegram_message(
    client: httpx.AsyncClient, bot_token: str, chat_id: str, message_id: int
):
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        for key in keys:
            ...
            if is_successful:
                ...
                logger.info(
                    f"Episode {anilist_id} Ep {episode_num} is SUCCESSFUL. Cleaning up progress key only."
                )
                upstash_del(key)
                continue
            ...
            logger.info(
                f"Episode {anilist_id} Ep {episode_num} is ORPHANED. Purging {len(progress_data)} segments from Telegram..."
            )
