...
async def _process_batch_impl(anilist_id: int, url: str):
        ...
        if file_ext == ".rar":
            ...
            if not os.path.exists(unrar_path):
                ...
                await asyncio.to_thread(
                    ...
                    "https://www.rarlab.com/rar/rarlinux-x64-621.tar.gz",
                    "/tmp/rar.tar.gz",
                )
            ...
            if process.returncode != 0:
                await log_status(
                    anilist_id,
                    f"❌ Gagal mengekstrak RAR dengan unrar dinamis: {stderr.decode()} | {stdout.decode()}",
                )
                return
        else:
            # zip fallback
    ...
    for file_path in mp4_files:
        ...
        m3u8_path = await slicer.slice(
            url=str(file_path), filename=file_path.name, provider_id="gdrive", segment_time=5
        )
        ...
        await log_status(
            anilist_id, f"⏳ Mengunggah potongan HLS Episode {ep_num} ke Telegram Swarm..."
        )
        progress_key = f"ingest_progress:{anilist_id}:{ep_num}"
        cloud_m3u8 = await uploader.process_hls_playlist_parallel(
            m3u8_path, progress_key=progress_key, max_workers=3
        )
    ...
    await log_status(anilist_id, "🎉 Batch Ingestion Selesai!")


if __name__ == "__main__":
    if len(sys.argv) < 3:
        print(
            "Penggunaan: python scripts/batch_gdrive_ingest.py <Anilist_ID> <Google_Drive/Desustream_URL>"
        )
        sys.exit(1)
