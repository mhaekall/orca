...
async def handle_check(client, chat_id, args):
    if len(args) == 0:
        status_msg = await send_message(
            client, chat_id, "🔍 Memuat daftar anime yang memiliki episode pending..."
        )
        try:
            ...
            if not rows:
                await send_message(
                    client,
                    chat_id,
                    "🎉 Hebat! Tidak ada anime dengan episode pending saat ini!",
            ...
            for r in rows:
                ...
                if len(title) > 35:
                    title = title[:35] + "..."
                keyboard.append(
                    [{"text": f"📺 {title}", "callback_data": f"check:{r['anilistId']}"}]
                )
    ...
    if len(args) != 1:
        await send_message(
            client,
            chat_id,
            "❌ Format salah. Gunakan: <code>/check</code> atau <code>/check &lt;anilist_id&gt;</code>",
        )
        return
    ...
    status_msg = await send_message(
        client, chat_id, f"🔍 Memeriksa database untuk Anime ID: <b>{anilist_id}</b>..."
    )
    ...
    try:
        ...
        if not rows:
            await send_message(
                client,
                chat_id,
                f"⚠️ Anime ID {anilist_id} tidak ditemukan di database (Belum di-sync).",
                message_id=status_msg,
            )
            return

        # Get clean title
        title_row = await database.fetch_one(
            'SELECT "cleanTitle" FROM anime_metadata WHERE "anilistId" = :aid',
        ...
        if pending_eps:
            ...
            keyboard.append(
                [
                    {
                        "text": f"🚀 AUTO INGEST SEMUA ({len(pending_eps)} Eps)",
                        "callback_data": f"auto:{anilist_id}",
                    }
                ]
            )
...
async def do_ingest(client, chat_id, anilist_id, ep_num, auto_mode=False):
    status_msg_id = None
    if not auto_mode:
        status_msg_id = await send_message(
            client, chat_id, f"⏳ [Ep {ep_num}] Memancing tautan mentah ke Cache..."
        )
    ...
    try:
        ...
        if not row:
            if not auto_mode:
                await send_message(
                    client,
                    chat_id,
                    f"⚠️ Episode {ep_num} tidak ditemukan di database.",
        ...
        if ep_url and ("tele-proxy" in ep_url or "workers.dev" in ep_url):
            if not auto_mode:
                await send_message(
                    client,
                    chat_id,
                    f"✅ [Ep {ep_num}] Sudah berstatus sukses (Bypass).",
                    message_id=status_msg_id,
                )
            return True
        ...
        if not direct_url:
            if not auto_mode:
                await send_message(
                    client,
                    chat_id,
                    f"❌ [Ep {ep_num}] Gagal memancing URL dari sumber.",
                    message_id=status_msg_id,
                )
            return False
        ...
        if not auto_mode:
            await send_message(
                client,
                chat_id,
                f"✅ [Ep {ep_num}] Tautan {quality_picked} diamankan ke Cache!\n🚀 Membangunkan GitHub Actions...",
                message_id=status_msg_id,
            )
        ...
        if not auto_mode:
            await send_message(
                client, chat_id, f"💥 Error Ingestion [Ep {ep_num}]: {e}", message_id=status_msg_id
            )
        return False
...
async def handle_auto(client, chat_id, args):
    if len(args) != 1:
        await send_message(
            client, chat_id, "❌ Format salah. Gunakan: <code>/auto &lt;anilist_id&gt;</code>"
        )
        return
    ...
    try:
        ...
        for r in rows:
            url = r["episodeUrl"]
            if not url or (
                "tele-proxy" not in url and "tg-proxy" not in url and "workers.dev" not in url
            ):
        ...
        if not pending:
            await send_message(
                client,
                chat_id,
                "✅ Tidak ada episode pending untuk di-ingest.",
                message_id=status_msg,
            )
            return
        ...
        await send_message(
            client,
            chat_id,
            f"🎣 Memancing {len(pending)} episode ke dalam Cache...",
            message_id=status_msg,
        )
        ...
        await send_message(
            client,
            chat_id,
            f"✅ <b>AUTO PANCING SELESAI!</b>\nBerhasil mengamankan <b>{success_count}/{len(pending)}</b> tautan mentah ke Cache.\n\n🚀 <b>GITHUB ACTIONS TELAH DIBANGUNKAN!</b>\nSemua episode tersebut sekarang sedang dikerjakan borongan oleh server Cloud. Silakan istirahat!",
            message_id=status_msg,
        )
        ...
        while True:
            try:
                res = await client.get(
                    f"{API_URL}/getUpdates", params={"offset": offset, "timeout": 30}
                )
                    ...
                    for update in data.get("result", []):
                        ...
                        if "message" in update and "text" in update["message"]:
                            ...
                            if text.startswith("/check"):
                                ...
                                if len(args) == 2:
                                    asyncio.create_task(
                                        do_ingest(client, chat_id, int(args[0]), float(args[1]))
                                    )
                                else:
                                    await send_message(
                                        client,
                                        chat_id,
                                        "❌ Format salah. Gunakan: <code>/ingest &lt;anilist_id&gt; &lt;episode&gt;</code>",
                                    )
                                ...
                                await send_message(
                                    client,
                                    chat_id,
                                    "👋 Halo Bos! Saya adalah <b>Orca Ingestion Bot</b>.\n\n"
