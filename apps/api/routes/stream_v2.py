...
async def _resolve_embed(embed: dict, source_tag: str) -> dict | None:
    ...
    try:
        ...
        is_direct = (
            embed.get("type") == "direct"
            or resolved.endswith((".m3u8", ".mp4"))
            or "videoplayback" in resolved
            or "workers.dev" in resolved
            or "tg-proxy" in resolved
        )
...
async def _scrape_samehadaku(
    title: str, episode_num: float, series_url: str = None, target_anilist_id: int = None
) -> dict[str, Any]:
    try:
        async with asyncio.timeout(PROVIDER_TIMEOUT):
            ...
            target_url = next(
                (
                    ...
                    if re.search(rf"\b{episode_num}\b", e["title"])
                ),
                None,
            )
            if not target_url:
    ...
    try:
        ...
        target_url = next(
            (
                e["url"]
                for e in details.get("episodes", [])
                if re.search(rf"\b{int(episode_num)}\b", e["title"])
            ),
            None,
        )
        if not target_url:
    ...
    try:
        async with asyncio.timeout(8.0):
            ...
            r = await otakudesu_provider.client.get(
                f"https://otakudesu.blog/?s={q}&post_type=anime"
            )
...
def _title_variants(title: str, anilist_info: dict = None) -> list[str]:
    ...
    cleaned = re.sub(
        r"\b(sub\s*indo|batch|bd|ova|season\s*\d+)\b", "", title, flags=re.IGNORECASE
    ).strip()
...
async def get_sources_v2(
            ...
            try:
                async with asyncio.timeout(10.0):
                    recon_result = await reconciler.reconcile(
                        provider_id="stream_query", provider_slug="none", raw_title=variant
                    )
    ...
    if mappings and "samehadaku" in mappings:
        ...
        scrape_tasks.append(
            _scrape_samehadaku(title_vars[0], ep, series_url, target_anilist_id=anilist_id)
        )
    else:
    ...
    if mappings and "otakudesu" in mappings:
        scrape_tasks.append(
            _scrape_otakudesu(f"https://otakudesu.blog/anime/{mappings['otakudesu']}/", ep)
        )
    else:
    ...
    if mappings and "kuronime" in mappings:
        ...
        scrape_tasks.append(
            _scrape_kuronime(title_vars[0], ep, series_url, target_anilist_id=anilist_id)
        )
    else:
    ...
    all_sources = [
        s for s in all_sources if s.get("type") in ["direct", "mp4 (direct)", "hls (direct)"]
    ]
