...
from db.models import anime_mappings, anime_metadata, episodes
...
async def check_db_consistency():
    ...
    try:
        ...
        query_mappings_no_meta = (
            select(func.count())
            .select_from(anime_mappings)
            .where(anime_mappings.c.anilistId.not_in(select(anime_metadata.c.anilistId)))
        )
        ...
        query_eps_no_meta = (
            ...
            .where(episodes.c.anilistId.not_in(select(anime_metadata.c.anilistId)))
        )
        ...
        if eps_no_mappings > 0:
            query_eps_no_mappings_ids = (
                select(episodes.c.anilistId.distinct())
                .where(episodes.c.anilistId.not_in(select(anime_mappings.c.anilistId)))
                .limit(10)
            )
            eps_no_mappings_ids = await database.fetch_all(query_eps_no_mappings_ids)
            print(
                f"  Sample anilistIds with eps but no mappings: {[r[0] for r in eps_no_mappings_ids]}"
            )
        ...
        query_meta_no_data = (
            select(func.count(anime_metadata.c.anilistId))
            .where(anime_metadata.c.anilistId.not_in(select(anime_mappings.c.anilistId)))
            .where(anime_metadata.c.anilistId.not_in(select(episodes.c.anilistId)))
        )
        meta_no_data = await database.execute(query_meta_no_data)
        print(
            f"Anime in metadata with NO mappings and NO episodes (Ghost metadata): {meta_no_data}"
        )
