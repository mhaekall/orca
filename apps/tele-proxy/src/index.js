  ...
  async fetch(request, env, ctx) {
    try {
      ...
      const cacheKey = new Request(cacheUrl.toString(), {
        method: "GET", // Cache put requires GET
      ...
      let responseFromOrigin = await fetch(modifiedRequest);
      
      if (!responseFromOrigin.ok && responseFromOrigin.status !== 206) {
         return new Response(responseFromOrigin.body, {
      ...
      if (url.pathname.endsWith('.m3u8') || url.searchParams.get("mime") === "m3u8" || responseFromOrigin.headers.get("Content-Type")?.includes("mpegurl")) {
        try {
          ...
          // If it was a binary file mistakenly caught here, response.text() will fail. Just return the raw body.
          body = responseFromOrigin.body;
        }
      }

      // 4. Rebuild headers for aggressive caching and CORS
      const newHeaders = new Headers(responseFromOrigin.headers);
      if (isModified) {
          newHeaders.delete("Content-Length");
      }
      ...
      } else if (responseFromOrigin.headers.get("Content-Type") === "application/octet-stream") {
        const contentLength = responseFromOrigin.headers.get("Content-Length");
      ...
      let finalStatus = responseFromOrigin.status;
      let finalStatusText = responseFromOrigin.statusText;
      ...
      return new Response("Worker Crash: " + err.stack, { status: 500 });
    }
  }
};