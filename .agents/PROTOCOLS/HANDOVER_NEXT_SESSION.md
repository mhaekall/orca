...
## 2. Completed Milestones (Fase 1-3)
...
- **Resume Video (Netflix-style):** Implemented a background `watch-session` ping every 30 seconds. When a user reopens a previously watched episode, `expo-video` automatically seeks to the exact saved timestamp (unless they finished >90% of the video). Keep-awake crashes were avoided by disabling `keepScreenOnWhilePlaying` in the `expo-video` config for now.
