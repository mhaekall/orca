    ...
    <div ref={ref} onMouseEnter={handleMouseEnter} onTouchStart={handleMouseEnter} className="flex flex-col h-full w-full group cursor-pointer anim-up" style={{ animationDelay: rank ? `${Math.min(rank * 40, 240)}ms` : "0ms" }}>
      <Link href={href} prefetch={true} className={`w-full ${aspectClass} rounded-2xl relative overflow-hidden mb-2 border border-white/5 bg-[#1c1c1e] block transition-shadow duration-300 group-hover:shadow-[0_8px_24px_rgba(0,0,0,0.5)] text-left focus:outline-none`}>
        {visible && imageSrc ? (
          <div className="absolute inset-0 w-full h-full transition-transform duration-500 group-hover:scale-105">
            <LiquidImage 
              ...
              fallbackSrc={img || "https://s4.anilist.co/file/anilistcdn/media/anime/cover/medium/default.jpg"}
            />
          </div>
        ) : (
        ...
        {!rank && currentBadge === "NEW" && (
          <span className="absolute top-2 left-2 px-1.5 py-0.5 bg-gradient-to-r from-[#FFCC00] to-[#FF9500] text-black text-[9px] font-black uppercase tracking-wider rounded shadow-md z-20">NEW</span>
        )}
        {!rank && currentBadge === "BEST" && (
          <span className="absolute top-2 left-2 px-1.5 py-0.5 bg-gradient-to-r from-[#32D74B] to-[#30D158] text-black text-[9px] font-black uppercase tracking-wider rounded shadow-md z-20">BEST</span>
        )}
        {!rank && currentBadge === "MOVIE" && (
          <span className="absolute top-2 left-2 px-1.5 py-0.5 bg-gradient-to-r from-[#BF5AF2] to-[#AF52DE] text-white text-[9px] font-black uppercase tracking-wider rounded shadow-md z-20">MOVIE</span>
        )}

        {/* Dynamic Episode display based on badge */}
        <span className="absolute top-2 right-2 px-1.5 py-0.5 bg-[#FF453A]/90 text-[9px] font-bold uppercase tracking-wider rounded z-20 shadow-md">
          {currentBadge === "MOVIE" ? "HD" : 
           currentBadge === "BEST" ? `${totalEps || epId || '?'} EPS` :
           `EPS ${epId || totalEps || '?'}`}
        </span>

        <div className="absolute bottom-2 left-2 right-2 z-20 flex items-center justify-between opacity-0 group-hover:opacity-100 translate-y-3 group-hover:translate-y-0 transition-all duration-200">
